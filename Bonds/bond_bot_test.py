...
async def send_report(context):
    ...
    for bond in bonds:
        ...
        income = calculate_income(price, avg_price)

        offer_date = parse_date_any(bond.get("Дата оферты")) or data.get("offer_date")

        rows.append({
            "Название": bond.get("Название"),
            "ISIN": isin,
            "Цена": safe_float(price),
            "YTM": safe_float(ytm),
            "Продать не ниже, в %": safe_float(bond.get("SellPrice")),
            "Купить не выше": safe_float(bond.get("BuyPrice")),
            "Депозиты Банка": bond.get("Депозиты Банка"),
            "Фио": bond.get("Фио"),
            "Рейтинг": bond.get("Рейтинг") or data.get("rating"),
            "Кол-во обл": safe_float(bond.get("Кол-во обл")),
            "Средняя цена": safe_float(avg_price),
            "Дата оферты": excel_date_or_none(offer_date),
